...
export default function AdminEngagementTools({ tenantId }: { tenantId: string }) {
  ...
  return (
    ...
    >
      ...
      <div className="rounded-lg border border-neutral-700 bg-black/30 p-4 space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          {data.clientLogin?.loginUrl ? (
            <a
              href={data.clientLogin.loginUrl}
              target="_blank"
              rel="noreferrer"
          ...
          <div>
            ...
            <div className="flex gap-2">
              <code className="flex-1 rounded-lg bg-black/40 border border-neutral-700 px-3 py-2 text-sm text-neutral-100 font-mono break-all">
                {data.clientLogin?.username || '— not set —'}
              </code>
              <button
                type="button"
                disabled={!data.clientLogin?.username}
                onClick={() => {
                  const v = data.clientLogin?.username
                  if (!v) return
