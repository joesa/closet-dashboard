import { NextResponse } from 'next/server'
import { actorTenantId, resolveDomainActor } from '@/lib/domains/auth'
import { makeDomainPrimary } from '@/lib/domains/manage'
import { checkRateLimit, hashRateKey } from '@/lib/rateLimit'
import { logAdminAction } from '@/lib/admin'

export const runtime = 'nodejs'

type Ctx = { params: Promise<{ id: string }> }

/** POST /api/domains/[id]/make-primary — set this hostname as the primary site URL. */
export async function POST(req: Request, ctx: Ctx) {
  const { id } = await ctx.params
  let body: Record<string, unknown> = {}
  try {
    body = await req.json()
  } catch {
    /* empty */
  }

  const tenantIdParam = typeof body.tenantId === 'string' ? body.tenantId : null
  const resolved = await resolveDomainActor({ tenantId: tenantIdParam })
  if ('error' in resolved) {
    return NextResponse.json({ error: resolved.error }, { status: resolved.status })
  }

  const tenantId = actorTenantId(resolved)
  if (!tenantId) {
    return NextResponse.json({ error: 'tenantId is required' }, { status: 400 })
  }

  const rateKey = hashRateKey(
    'domains-primary',
    resolved.actor.role === 'admin' ? resolved.actor.adminId : resolved.actor.userId
  )
  const rate = await checkRateLimit(rateKey, 20, 10 * 60_000)
  if (!rate.allowed) {
    return NextResponse.json({ error: 'Too many requests' }, { status: 429 })
  }

  try {
    const domain = await makeDomainPrimary(tenantId, id)
    if (resolved.actor.role === 'admin') {
      await logAdminAction({
        actor: { id: resolved.actor.adminId, email: resolved.actor.email },
        action: 'domain.make_primary',
        targetType: 'domain',
        targetId: id,
        metadata: { tenantId, hostname: domain.hostname },
      })
    }
    return NextResponse.json({ success: true, domain })
  } catch (e) {
    console.error('make-primary error:', e)
    return NextResponse.json(
      { error: e instanceof Error ? e.message : 'Failed to update primary domain' },
      { status: 400 }
    )
  }
}
