...
- process: a 3-step how-it-works section. The steps array MUST contain exactly 3 steps with internal ordering keys '01', '02', '03' in that exact sequence; these keys are schema metadata and are never visitor-facing copy or visual labels. The title and subtitle MUST reflect the actual trade — do NOT use "Our Architectural Process" or "From Vision to Flawless Reality" unless the business is literally an architecture or design firm. Example adaptations: drain cleaning → "Book → Diagnose → Fix"; HVAC → "Assess → Recommend → Install"; roofing → "Inspect → Estimate → Install"; closets → "Design → Build → Install". Each step is a vivid one-sentence description.
