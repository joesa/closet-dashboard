...
#
# ...or, before this branch is merged, scp it over and `bash oracle-vm-bootstrap.sh`.
#
...
# Docker Engine from Docker's own repo. Ubuntu's docker.io package lags and does
# not ship the compose v2 plugin that docker-compose.prod.yml needs.
...
# Survives reboots. Oracle images ship Docker disabled by default when installed
# this way, and a worker that does not come back after a reboot is the whole
# failure mode this migration has to avoid.
